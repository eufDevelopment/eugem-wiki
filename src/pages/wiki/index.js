  ...
  if(localImages) {
    for(let i=0; i<localImages.length; i++){
      ...
      if(lImage.hasOwnProperty('url') && lImage.hasOwnProperty('childImageSharp')){
        ...
        newBodyHtml = newBodyHtml.replace(imgUrl,lImage.childImageSharp.original.src).replace(imgUrl.replace('/attachments/','/thumbnails/'),lImage.childImageSharp.original.src)
      }
    }
  }
  return (
    <Layout>
      ...
      <small>
        {ancestors.map(ancestor => (
            ...
            <Link 
              to={`/wiki/${ancestor.id}`}
              className={ancestor.id === `4423682` ? `hidden` : `crumbly`}  
              style={{
                textDecoration: "none",
                margin: "6px 6px 0 0",
              }}
